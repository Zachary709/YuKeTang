...
- **自动刷测试题**：
  ...
  - 对于填空题很多的课，可以把自己找到的答案**格式严格整理好**，答案用两个“*”夹着，每个题只占一行，然后放到answer/下，里面有一份模板，之后所有题都会自动提交。（**该功能不是很完善**，不建议使用，填空题多的课，最好还是手刷）。
...
- `LLM_MODEL`：用于生成评论的模型名称，如 `qwen3.6-max-preview`，可根据你的实际服务调整。

API KEY需要自己申请一下，建议使用阿里云百炼，这样模型和URL都可以直接用这里默认的。（截至2026年5月，qwen3.6-max-preview还是有免费额度的，不用考虑付费的事）
